...
if page == "Accueil":
    ...
    st.title("Portfolio Data Analyse d'Elise Le Chevillier")

    st.markdown("""
    Bienvenue sur mon portfolio.
""")
    
elif page == "Projet Parcoursup":

    st.set_page_config(
    page_title="Projet Parcoursup",
    layout="wide"
)
    ...
    with col2:
        ...
        fig1.update_layout(
            title={
                'text': "Types de formations par nombre de candidats",
                'x': 0.5,
                'xanchor': 'center'
            },
    ...
    with col1:
        ...
        labels_courts = [
            nom[:55] + "..."
            if len(nom) > 55 else nom
            for nom in top_formations_detail.index
        ]
    ...
    with col2:
        ...
        lambda t: t.update(
            name=t.name
            .replace("capacite_formation", "Capacité d'accueil")
            .replace("effectif_total_candidats", "Effectif total des candidats")
        )
        )
    ...
    with col1:
        ...
        fig.update_layout(
            title={
                'text': "Candidatures et admissions par genre",
                'x': 0.5,
                'xanchor': 'center'
            },
            showlegend=False,
    ...
    with col2:
        ...
        fig.update_layout(
            title={
                    'text': "Répartition des candidatures et admissions par genre et formation",
                    'x': 0.5,
                    'xanchor': 'center'
                },
            xaxis_tickangle=-45,
        ...
        lambda t: t.update(
            name=t.name
            .replace("pct_femmes_candidats", "% de femmes candidates")
            .replace("pct_hommes_candidats", "% des hommes candidats")
            .replace("pct_femmes_admis", "% des femmes admises")
            .replace("pct_hommes_admis", "% des hommes admises")
        )
        )
    ...
    with col1:
        ...
        fig.update_layout(
            title={
                        'text': f"Top 10 des régions avec le plus de formations",
                        'x': 0.5,
                        'xanchor': 'center'
                    },
                yaxis={'categoryorder':'total ascending'},
    ...
    fig.update_layout(
        title={
                        'text': f"Heatmap nombre de formations par région",
                        'x': 0.5,
                        'xanchor': 'center'
                    },
        xaxis_tickangle=-45,
    ...
    with col1:
        ...
        fig.update_layout(
            title={
                            'text': f"Admis PCV vs autres (formations en IDF: Paris / Créteil / Versailles)",
                            'x': 0.5,
                            'xanchor': 'center'
                        },
            xaxis_title=None,
        ...
        lambda t: t.update(
            name=t.name
            .replace("admis_pcv", "Admis originaires des académies IDF")
            .replace("admis_autres", "Admis originaires d'autres académies")
            )
        )
    ...
    with col2:
        ...
        fig.update_layout(
            title={
                            'text': f"Admis PCV vs autres (toutes formations)",
                            'x': 0.5,
                            'xanchor': 'center'
                        },
            xaxis_title=None,
    ...
    with col2:
        ...
        fig.update_layout(
            title={
                            'text': f"Répartition des admis par type de bac selon la formation",
                            'x': 0.5,
                            'xanchor': 'center'
                        },
            xaxis_title=None,
    ...
    with col1:
        ...
        fig.update_layout(
            title={
                            'text': f"Répartition des admis par type de mention (%)",
                            'x': 0.5,
                            'xanchor': 'center'
                        },
            xaxis_title=None,
    ...
    with col2:
        ...
        fig.update_layout(
            title={
                            'text': f"Part des admis néobacheliers boursiers vs non boursiers (%)",
                            'x': 0.5,
                            'xanchor': 'center'
                        },
            xaxis_title=None,
        ...
        lambda t: t.update(
            name=t.name
            .replace("dont_effectif_des_admis_boursiers_neo_bacheliers", "Admis boursiers")
            .replace("non_boursiers", "Admis non-boursiers")
            )
        )
